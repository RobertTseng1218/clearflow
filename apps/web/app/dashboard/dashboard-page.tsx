...
import { bindDataUpdated, getDashboard, getIntegrations, getPlan, getUsage } from '@/lib/api';
...
function highlightIdentity(item: any) {
  const sourceType = itemSourceType(item) || 'unknown';
  if (item?.related_id) return `${sourceType}:${item.related_id}`;
  return `${sourceType}:title:${String(item?.title || '').trim().toLowerCase()}`;
}

function dedupeHighlights(items: any[]) {
  const seen = new Set<string>();
  return items.filter((item) => {
    const key = highlightIdentity(item);
...
function todaySourceOverviewText(sourceOverview?: any, sourceBreakdown?: Record<string, number>) {
  const labels = sourceOverview?.today_data_source_labels as string[] | undefined;
  if (labels && labels.length > 0) {
    return `今天有資料來自 ${labels.join('、')}`;
  }

  const breakdown = sourceBreakdownText(sourceBreakdown);
  if (breakdown) {
    return `今天有資料來自 ${breakdown.replace(/｜/g, '、')}`;
  }

  return '今天尚未同步到新的資料來源';
}
  ...
  const load = async (options?: { silent?: boolean; reason?: 'sync' | 'task' | 'manual' }) => {
    ...
    try {
      ...
      if (options?.reason === 'sync') {
        setRefreshNotice('同步結果已更新到總覽。');
      } else if (options?.reason === 'task') {
        setRefreshNotice('待辦狀態已更新到總覽。');
      }
    } catch (err) {
      const message = err instanceof Error ? err.message : '資料載入失敗';

      if (silent && dashboard) {
        setRefreshNotice('總覽更新未完成，先顯示上次結果。');
      } else {
        setError(message);
      }
    } finally {
